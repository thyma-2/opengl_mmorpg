...
void send_unit_list(int socket)
{
    ...
    for (struct unit *u = unitlist; u != NULL; u = u->next)
		sprintf (ordre + strlen(ordre), "%d %d %s %s %s %s %f %f %f %f %f %f\n", u->id, u->pv, u->acount, u->name, u->utype, u->bossname, u->x, u->y, u->z, u->rx, u->ry, u->rz);
	int s = strlen(ordre);
	if (s > 0)
	{
		sprintf (order, "%d", s);
...
int generate_order(char *order)
{
	...
	for (struct unit *u = unitlist; u != NULL; u = u->next)
	{
		if (u->has_changed == 1)
		{
			...
			sprintf (to_write + strlen(to_write), "%d %d %s %s %s %s %f %f %f %f %f %f\n", u->id, u->pv, u->acount, u->name, u->utype, u->bossname, u->x, u->y, u->z, u->rx, u->ry, u->rz);
			u->has_changed = 0;
		}	
	}
